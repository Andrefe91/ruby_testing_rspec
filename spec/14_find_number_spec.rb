...
describe FindNumber do
  ...
  describe '#update_range' do
    ...
    context "when the guess is one more than the max" do
      let(:number_range) { double('random_number', value: 5) }
      subject(:edge_guess_game) { described_class.new(0, 6, number_range, 7) }

      it "doesnt change the max value" do
        expect(edge_guess_game.max).to eq 6
      end
    end
  end
end
