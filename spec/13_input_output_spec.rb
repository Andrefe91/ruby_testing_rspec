...
describe NumberGame do
  ...
  describe '#game_over?' do
    ...
    context 'when user guess is not correct' do
      subject(:game_fail) { described_class.new(5, '4') }
      # remove the 'x' before running this test
      it 'is not game over' do
        expect(game_fail).to_not be_game_over
      end
    end
  end
  ...
  describe '#player_turn' do
    ...
    context 'when user inputs two incorrect values, then a valid input' do
      before do
        ...
        invalid_input_2 = "e"
        valid_input = "3"
        allow(game_loop).to receive(:player_input).and_return(invalid_input_1, 
        invalid_input_2, valid_input)
      end
  ...
  describe '#final_message' do
    ...
    context 'when count is 4 and over' do
      subject(:game_over) { described_class.new(3, '3', 5)}
      # remove the 'x' before running this test
      it 'outputs correct phrase' do
        sorry_phrase = "That was hard. It took you 5 guesses!\n"
        expect {game_over.final_message}.to output(sorry_phrase).to_stdout
      end
    end
  end
end
