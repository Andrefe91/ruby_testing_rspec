...
describe BinaryGame do
  ...
  describe '#player_input' do
    ...
    context 'when user inputs an incorrect value once, then a valid input' do
      before do
        invalid_input_1 = '12'
        valid_input_1 = '9'
        allow(game_input).to receive(:gets).and_return(invalid_input_1, valid_input_1)
      end
    ...
    context 'when user inputs two incorrect values, then a valid input' do
      before do
        ...
        invalid_input_2 = '25'
        valid_input_1 = '9'
        allow(game_input).to receive(:gets).and_return(invalid_input_1,
          invalid_input_2, valid_input_1)
      end
  ...
  describe '#verify_input' do
    ...
    context 'when given a valid input as argument' do
      it 'returns valid input' do
        ...
        input = 5
        verified_input = game_verify_input.verify_input(min, max, input)
  ...
  describe '#display_turn_order' do
    ...
    let(:search_display_turn_order) { instance_double(BinarySearch) }
    subject(:game_display_turn_order) {described_class.new(1,100, search_display_turn_order) }

    before do
      # You'll need to create a few method stubs.
      allow(search_display_turn_order).to receive(:make_guess)
      allow(search_display_turn_order).to receive(:update_range)
      allow(game_display_turn_order).to receive(:display_guess)
      allow(search_display_turn_order).to receive(:game_over?)
    end

    # Command Method -> Test the change in the observable state
    it 'increases guess_count by one' do
      expect{game_display_turn_order.display_turn_order(search_display_turn_order)}.to change {game_display_turn_order.instance_variable_get(:@guess_count)}.by 1
    end

    # Method with Outgoing Command -> Test that a message is sent
    it 'sends make_guess' do
      expect(search_display_turn_order).to receive(:make_guess)
      game_display_turn_order.display_turn_order(search_display_turn_order)
    end

    # Method with Outgoing Command -> Test that a message is sent
    it 'sends update_range' do
      expect(game_display_turn_order).to receive(:display_guess).with(search_display_turn_order)
      game_display_turn_order.display_turn_order(search_display_turn_order)
    end
